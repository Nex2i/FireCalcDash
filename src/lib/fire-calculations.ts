...
export function calculateFire(inputs: FireInputs): FireCalculationResult {
  ...
  for (let year = 0; year < 40 && age <= 100; year++) {
    ...
    const fireTarget = realFireNumber * Math.pow(1 + inflationRate / 100, yearsFromNow);
    
    // Store starting balance for the year
    const startingBalance = currentInvestments;
    
    // Calculate annual contributions (monthly * 12), adjusted for inflation if enabled
    const baseAnnualContribution = monthlyContributions * 12;
    const annualContribution = adjustContributionsForInflation 
      ? baseAnnualContribution * Math.pow(1 + inflationRate / 100, yearsFromNow)
    ...
    const potentialWithdrawalNominal = currentInvestments * (withdrawalRate / 100);
    // Real withdrawal is the nominal amount adjusted back to today's purchasing power
    const potentialWithdrawalReal = potentialWithdrawalNominal / Math.pow(1 + inflationRate / 100, yearsFromNow);
    ...
    projectionData.push({
      ...
      potentialWithdrawalNominal: Math.round(potentialWithdrawalNominal / 1000) * 1000,
      potentialWithdrawalReal: Math.round(potentialWithdrawalReal / 1000) * 1000,
      fireTarget: Math.round(fireTarget / 1000) * 1000,
      windfallAmount: windfallAmount > 0 ? Math.round(windfallAmount / 1000) * 1000 : undefined,
  ...
  const nominalFireNumber = realFireNumber * Math.pow(1 + inflationRate / 100, yearsToRetirement);
